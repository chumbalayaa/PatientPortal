...
// e.g. jane_goodall mood
    function deleteAllGraphs(previousPatient) {
      for (var i = 0; i < Patients[previousPatient]['forms'].length; i ++ ) {
        var firstName = Patients[previousPatient]['firstName'].toLowerCase();
        var lastName = Patients[previousPatient]['lastName'].toLowerCase();
        var patientName = firstName.concat('_').concat(lastName);
        var div_name = patientName.concat('_').concat(Patients[previousPatient]['forms'][i]); 
    ...
    function drawGraph(fullName, graphType, startDate, endDate, minDate, maxDate) {
        ...
        if ((fullName != "JaneGoodall") && (fullName != "MarshallMathers") ) {
          dataName = "JaneGoodall";
        } 
        firstName = Patients[fullName]['firstName'].toLowerCase();
        lastName = Patients[fullName]['lastName'].toLowerCase();
        var patientName = firstName.concat('_').concat(lastName);
        var div_name = patientName.concat('_').concat(graphType);
        firstName2 = Patients[dataName]['firstName'].toLowerCase();
        lastName2 = Patients[dataName]['lastName'].toLowerCase();
        var patientName2 = firstName2.concat('_').concat(lastName2);
        var div_name2 = patientName2.concat('_').concat(graphType); 
        ...
        addTitle(div_name, graphType);
        addDownloadButton(div_name2);
        ...
        d3.csv("data/".concat(div_name2).concat(".txt"), function(error, data) {
            ...
            //     ANNOTATION_ON = true;
                ...
                coordinates = d3.mouse(this);
                 ...
                 if (isFlag(main_x(d.TimeOfDay))) { // on the flag
                   ...
                   var delete_annotation = document.getElementById("delete-annotation");
                    delete_annotation.onclick = function(evt){
                      deleteAnnotation(capitalizeFirstLetter(firstName), capitalizeFirstLetter(lastName), graphType, d.TimeOfDay.toDateString());
                      makeGraph(fullName, graphType, startDate, endDate, minDate, maxDate);
                    }; 
...
function addTitle(div_name, graphType) {
  var formatted_graph_type = capitalizeFirstLetter(graphType).concat(" Graph");
  $("#".concat(div_name)).append('<div class="title"><h1>'.concat(formatted_graph_type).concat('</h1></div>'));
}
...
function addDateRange(div_name, fullName, graphType, startDate, endDate, minDate, maxDate) {
  ...
  $("#".concat(div_name)).append('<div class="startdate-picker"><div>Beginning of date range: <input type="text" placeholder="'.concat(formatted_startdate ).concat('" name="selected_date" id="')
    .concat(startdate_name).concat('" /></div>').concat(' <div> End of date range: <input type="text" placeholder="').concat(formatted_enddate ).concat('" name="selected_date" id="').concat(enddate_name).concat('" /></div></div>'));
  ...
  $(function(){
    ...
    //   } 
    // });
    $("#".concat(startdate_name)).pickadate({
      min: minDate, 
      max: endDateMinus,
      onSet: function(context) {
        var updatedDate = this.get('select')['obj']
    ...
    // $("#".concat(startdate_name)).datepicker('show');
